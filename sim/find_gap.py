"""
 Coplanar Waveguide (CPW) Line Test — CPW Gap Sweep
 ...
 Ground-via spacing is held constant at 2000 um. This version sweeps the
 CPW gap from 0.1 mm to 0.3 mm in 0.02 mm steps, running a full FDTD
 simulation for each gap value and saving the S11 and S21 curves as
 separate PNG images per gap value:

     S11_CPW_GAP=<gap>um.png
     S21_CPW_GAP=<gap>um.png

 into ./cpw_gap_sweep/

 NOTE: this runs 11 complete openEMS FDTD simulations back-to-back (one
 per gap value), so total runtime will be roughly 11x a single run.
...
def run_simulation(cpw_gap):
    """Build geometry, run FDTD, and return (f, s11_dB, s21_dB) for a given CPW_gap (um).
    via_spacing is held fixed at 2000 um for this sweep."""

    print(f'epsilon_0 is {epsilon_0}')

    ### Setup the simulation
    Sim_Path = os.path.join(tempfile.gettempdir(), f'CPW_Line_gap_{int(round(cpw_gap))}um')

    unit                = 1e-6   # drawing unit in um
    CPW_length          = 40000
    CPW_port_length     = 10000
    CPW_width           = 620
    CPW_gap             = cpw_gap  # swept parameter (um)
...
def save_plot(f, data_dB, label, cpw_gap, out_dir):
    """Save a single S-parameter curve to <label>_CPW_GAP=<gap>um.png"""
    fig, axis = plt.subplots(num=f'{label}_gap_{int(round(cpw_gap))}um', tight_layout=True)
    ...
    axis.set_title(f'CPW_gap = {int(round(cpw_gap))} um  (via_spacing = 2000 um)')
    axis.legend()

    fname = out_dir / f'{label}_CPW_GAP={int(round(cpw_gap))}um.png'
...
if __name__ == '__main__':
    ...
    out_dir = pathlib.Path('./cpw_gap_sweep').resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    # Sweep CPW_gap from 0.1mm to 0.3mm in 0.02mm steps (inclusive of both ends -> 11 runs).
    # Built in mm first (matching how the sweep was specified) then converted to um,
    # since the model's drawing unit is 1e-6 (um).
    cpw_gap_mm = np.round(np.arange(0.1, 0.3 + 1e-9, 0.02), 2)
    cpw_gaps_um = cpw_gap_mm * 1000  # e.g. 100, 120, ..., 300 um

    for cpw_gap in cpw_gaps_um:
        print(f'\n=== Running simulation for CPW_gap = {cpw_gap:.0f} um ({cpw_gap/1000:.2f} mm), via_spacing = 2000 um ===')
        f, s11_dB, s21_dB = run_simulation(float(cpw_gap))
        ...
        save_plot(f, s11_dB, 'S11', cpw_gap, out_dir)
        save_plot(f, s21_dB, 'S21', cpw_gap, out_dir)

    print('\nAll simulations complete. PNGs saved in ./cpw_gap_sweep/')