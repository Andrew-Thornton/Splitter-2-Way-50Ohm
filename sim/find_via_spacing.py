"""
 Coplanar Waveguide (CPW) Line Test — Via Spacing Sweep
 ...
 This version sweeps the ground-via spacing from 1000 um to 4000 um in
 100 um steps, running a full FDTD simulation for each spacing and saving
 the S11 and S21 curves as separate PNG images per spacing value:

     S11_via_spacing_<spacing>um.png
     S21_via_spacing_<spacing>um.png

 into ./via_spacing_sweep/

 NOTE: this runs 31 complete openEMS FDTD simulations back-to-back (one
 per spacing value), so total runtime will be roughly 31x a single run.
...
def run_simulation(via_spacing):
    """Build geometry, run FDTD, and return (f, s11_dB, s21_dB) for a given via_spacing (um)."""

    print(f'epsilon_0 is {epsilon_0}')

    ### Setup the simulation
    Sim_Path = os.path.join(tempfile.gettempdir(), f'CPW_Line_via_{int(round(via_spacing))}um')
    ...
    via_radias = 500 #um
    # via_spacing is now a function parameter (swept from 1000um to 4000um in 100um steps)
...
def save_plot(f, data_dB, label, via_spacing, out_dir):
    """Save a single S-parameter curve to <label>_via_spacing_<spacing>um.png"""
    fig, axis = plt.subplots(num=f'{label}_{int(round(via_spacing))}um', tight_layout=True)
    ...
    axis.set_title(f'via_spacing = {int(round(via_spacing))} um')
    axis.legend()

    fname = out_dir / f'{label}_via_spacing_{int(round(via_spacing))}um.png'
...
if __name__ == '__main__':
    ...
    out_dir = pathlib.Path('./via_spacing_sweep').resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    # Sweep via_spacing from 1000um to 4000um in 100um steps (inclusive of both ends -> 31 runs)
    via_spacings = np.arange(1000, 4000 + 1, 100)

    for via_spacing in via_spacings:
        print(f'\n=== Running simulation for via_spacing = {via_spacing} um ===')
        f, s11_dB, s21_dB = run_simulation(float(via_spacing))
        ...
        save_plot(f, s11_dB, 'S11', via_spacing, out_dir)
        save_plot(f, s21_dB, 'S21', via_spacing, out_dir)

    print('\nAll simulations complete. PNGs saved in ./via_spacing_sweep/')
